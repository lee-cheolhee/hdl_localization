...
class HdlLocalizationNodelet : public nodelet::Nodelet
{
    ...
    void onInit() override
    {
        ...
        if (use_global_localization)
        {
            ...
            query_global_localization_service =
                nh.serviceClient<hdl_global_localization::QueryGlobalLocalization>("/hdl_global_localization/query");
  ...
  private:
    pcl::Registration<PointT, PointT>::Ptr create_registration() const
    {
        ...
        std::string fastvgicpCUDA_nearest_neighbor_search_method =
            private_nh.param<std::string>("fastvgicpCUDA_nearest_neighbor_search_method", "CPU_PARALLEL_KDTREE");
        ...
        else if (reg_method.find("FastVGICP_CUDA") != std::string::npos)
        {
            ...
            if (reg_method.find("PLANE") != std::string::npos)
            { //见fast_vgicp_cuda_impl.hpp
  ...
  private:
    ...
    void points_callback(const sensor_msgs::PointCloud2ConstPtr& points_msg)
    {
        ...
        if (private_nh.param<bool>("enable_robot_odometry_prediction", false) && !last_correction_time.isZero())
        {
            ...
            if (tf_buffer.canTransform(odom_child_frame_id, last_correction_time, odom_child_frame_id, stamp, robot_odom_frame_id,
                                       ros::Duration(0.1)))
            {
            ...
            else if (tf_buffer.canTransform(odom_child_frame_id, last_correction_time, odom_child_frame_id, ros::Time(0),
                                            robot_odom_frame_id, ros::Duration(0)))
            {
                odom_delta = tf_buffer.lookupTransform(odom_child_frame_id, last_correction_time, odom_child_frame_id, ros::Time(0),
                                                       robot_odom_frame_id, ros::Duration(0));
            }
    ...
    bool relocalize(std_srvs::EmptyRequest& req, std_srvs::EmptyResponse& res)
    {
        ...
        pose.linear() =
            Eigen::Quaternionf(result.orientation.w, result.orientation.x, result.orientation.y, result.orientation.z).toRotationMatrix();
        ...
        pose_estimator.reset(new hdl_localization::PoseEstimator(registration, pose.translation(), Eigen::Quaternionf(pose.linear()),
                                                                 private_nh.param<double>("cool_time_duration", 0.5)));
    ...
    void publish_scan_matching_status(const std_msgs::Header& header, pcl::PointCloud<pcl::PointXYZI>::ConstPtr aligned)
    {
        ...
        if (pose_estimator->wo_prediction_error())
        {
            ...
            status.prediction_errors.push_back(
                tf2::eigenToTransform(Eigen::Isometry3d(pose_estimator->wo_prediction_error().get().cast<double>())).transform);
        }
        ...
        if (pose_estimator->imu_prediction_error())
        {
            ...
            status.prediction_errors.push_back(
                tf2::eigenToTransform(Eigen::Isometry3d(pose_estimator->imu_prediction_error().get().cast<double>())).transform);
        }
        ...
        if (pose_estimator->odom_prediction_error())
        {
            ...
            status.prediction_errors.push_back(
                tf2::eigenToTransform(Eigen::Isometry3d(pose_estimator->odom_prediction_error().get().cast<double>())).transform);
        }
